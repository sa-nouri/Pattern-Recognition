...
def k_nearest_neighbors(x, td, k):
    dists= np.sqrt(np.sum((td-x)**2, axis=1))
    k_min_labels= []
    for j in range(k):
        min_idx= np.argmin(dists)
        k_min_labels.append(train_labels[min_idx])
        dists[min_idx]= float('inf')
    return Counter(k_min_labels).most_common()[0][0]
...
with timer():
    for j in range(test_labels.shape[0]):
        decision_labels[j]= k_nearest_neighbors(test_data[j], train_data, k)

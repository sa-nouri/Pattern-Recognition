...
def get_class_label(data, mu, cov, priors, num_of_nums):
    ...
    for i in range(num_of_nums):
        if ((priors[i] * multivariate_normal.pdf(data, mu[i], cov))>=tmp):
            tmp= (priors[i] * multivariate_normal.pdf(data, mu[i], cov))
            ans= i
    return ans

def get_class_label_parzen(data, priors, num_of_nums, win_type):
    ans= -1
    liklihoods= np.zeros(shape=(num_of_nums, len(test_labels)))
    for i in range(num_of_nums):
        print(i)
        td= train_data[np.where(train_labels==i)[0]]
        liklihoods[i]= priors[i] * parzen_pdf_estimation(data, td, i, win_type)
    ans = np.argmax(liklihoods, axis=0)
    return ans

def parzen_pdf_estimation(x, td, i, win_type):
    n= len(np.where(train_labels==i)[0])
    scaled_vals= (x[:,np.newaxis,:] - td)/win_len
    if (win_type=='rectangular'):
        win_vals= np.all(np.abs(scaled_vals)<0.5, axis=2)
    elif (win_type=='gaussian'):
        win_vals= multivariate_normal.pdf(scaled_vals, np.zeros(shape=(train_data.shape[1])))
    ans= np.sum(win_vals/n, axis=1)
    return ans
...
with timer():
    decision_labels= get_class_label_parzen(test_data, priors, num_of_nums, 'gaussian')

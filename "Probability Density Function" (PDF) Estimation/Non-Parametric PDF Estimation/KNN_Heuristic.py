...
def get_class_label_knn(data, priors, num_of_nums, k):
    liklihoods= np.zeros(shape=(num_of_nums, 1))
    for i in range(num_of_nums):
        liklihoods[i]= priors[i] *  knn_pdf_estimation(data, train_data, i, k)
    ans = np.argmax(liklihoods)
    return ans

def knn_pdf_estimation(x, td, i, k):
    idx= np.where(train_labels==i)[0]
    dists= np.sqrt(np.sum((td[idx]-x)**2, axis=1))
    for j in range(k):
        min_idx= np.argmin(dists)
        k_min_value= dists[min_idx]
        dists[min_idx]= float('inf')
    return -math.log(len(idx))-feat_size*math.log(k_min_value)
...
with timer():
    for j in range(test_labels.shape[0]):
        decision_labels[j]= get_class_label_knn(test_data[j], priors, num_of_nums, k)
